 ...
 * @author joelai
 *
 * @file /algae-bp/package/tester1/tester_rtsp2.cpp
 * @brief tester_rtsp2
 */
...
int live555_main(int argc, const char **argv) {
...
#  endif
	...
	// A H.265 video elementary stream:
	{
		...
		ServerMediaSession *sms = ServerMediaSession::createNew(*env,
				streamName, streamName, descriptionString);
		sms->addSubsession(
				H265VideoFileServerMediaSubsession::createNew(*env,
						inputFileName, reuseFirstSource));
		rtspServer->addServerMediaSession(sms);
	...
	// A MPEG-1 or 2 video elementary stream:
	{
		...
		ServerMediaSession *sms = ServerMediaSession::createNew(*env,
				streamName, streamName, descriptionString);
		sms->addSubsession(
				MPEG1or2VideoFileServerMediaSubsession::createNew(*env,
						inputFileName, reuseFirstSource, iFramesOnly));
		rtspServer->addServerMediaSession(sms);
...
//#define INTERLEAVE_ADUS 1
...
#endif
		sms->addSubsession(
				MP3AudioFileServerMediaSubsession::createNew(*env,
						inputFileName, reuseFirstSource, useADUs,
						interleaving));
		rtspServer->addServerMediaSession(sms);
	...
	// A WAV audio stream:
	{
		...
		sms->addSubsession(
				WAVAudioFileServerMediaSubsession::createNew(*env,
						inputFileName, reuseFirstSource, convertToULaw));
		rtspServer->addServerMediaSession(sms);
	...
	// An AMR audio stream:
	{
		...
		ServerMediaSession *sms = ServerMediaSession::createNew(*env,
				streamName, streamName, descriptionString);
		sms->addSubsession(
				AMRAudioFileServerMediaSubsession::createNew(*env,
						inputFileName, reuseFirstSource));
		rtspServer->addServerMediaSession(sms);
	...
	// An AAC audio stream (ADTS-format file):
	{
		...
		ServerMediaSession *sms = ServerMediaSession::createNew(*env,
				streamName, streamName, descriptionString);
		sms->addSubsession(
				ADTSAudioFileServerMediaSubsession::createNew(*env,
						inputFileName, reuseFirstSource));
		rtspServer->addServerMediaSession(sms);
	...
	// A DV video stream:
	{
		...
		ServerMediaSession *sms = ServerMediaSession::createNew(*env,
				streamName, streamName, descriptionString);
		sms->addSubsession(
				DVVideoFileServerMediaSubsession::createNew(*env, inputFileName,
						reuseFirstSource));
		rtspServer->addServerMediaSession(sms);
	...
	// A AC3 video elementary stream:
	{
		...
		sms->addSubsession(
				AC3AudioFileServerMediaSubsession::createNew(*env,
						inputFileName, reuseFirstSource));
	...
	// A Matroska ('.mkv') file, with video+audio+subtitle streams:
	{
		...
		MatroskaFileServerDemux::createNew(*env, inputFileName,
	...
	// An Ogg ('.ogg') file, with video and/or audio streams:
	{
		...
		newDemuxWatchVariable = 0;
		OggFileServerDemux::createNew(*env, inputFileName, onOggDemuxCreation,
				NULL);
		env->taskScheduler().doEventLoop(&newDemuxWatchVariable);
	...
	// A MPEG-2 Transport Stream, coming from a live UDP (raw-UDP or RTP/UDP) source:
	{
		...
		ServerMediaSession *sms = ServerMediaSession::createNew(*env,
				streamName, streamName, descriptionString);
		sms->addSubsession(
				MPEG2TransportUDPServerMediaSubsession::createNew(*env,
