...
static int lfb_open(const char *dev, lfb_t *lfb) {
	...
	if (lfb) {
	    ...
	    log_d("%d x %d x %dbits\n", lfb->fbvinfo.xres, lfb->fbvinfo.yres,
	    		lfb->fbvinfo.bits_per_pixel);
...
static int bmp_open(const char *dev, bmp_t *bmp, char preload) {
	...
	log_d("%d x %d x %dbits\n", bmp->bhdr->biWidth, bmp->bhdr->biHeight,
			bmp->bhdr->biBitCount);
...
static int lfb_draw_bmp(lfb_t *lfb, bmp_t *bmp) {
	...
	uint8_t *lfb_row;
	size_t row_sz = aloe_min(bmp->stride, lfb->stride), bmp_stride, bmp_height;

	if (opt_bmp_height > 0) {
		bmp_height = aloe_min(abs(bmp->bhdr->biHeight), opt_bmp_height);
	} else {
...
void* tester_fb1_thread(void *args) {
	const char *argv[] = {
		"fb1"
	};
	int argc = aloe_arraysize(argv);

	return (void*)(unsigned long)(*test_case.run)(1, argc, argv);
}
