 ...
 * @author joelai
 *
 * @file /algae-bp/package/tester1/tester_v4l2rtsp.cpp
...
private:
  ...
  static void v4l2ReadableHandler(V4l2H264FramedSource* source, int /*mask*/) {
    ...
    if (encSize > 0 && !encodedData.empty()) {
      gPipeline->frameQueue.push_back(std::move(encodedData));
      while (gPipeline->frameQueue.size() > 3) {
        gPipeline->frameQueue.pop_front();
      }
    }
  ...
  Boolean deliverQueuedFrame() {
    if (gPipeline->frameQueue.empty()) return False;

    std::vector<uint8_t> frame = std::move(gPipeline->frameQueue.front());
    gPipeline->frameQueue.pop_front();
    ...
    memmove(fTo, frame.data(), fFrameSize);
