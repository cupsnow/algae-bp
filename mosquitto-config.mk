...
APP_OPENSSL_API_COMPAT=0x10100000L

ifeq ($(WITH_TLS),yes)
	APP_CPPFLAGS:=$(APP_CPPFLAGS) -DWITH_TLS $(APP_OPENSSL_API_COMPAT:%=-DOPENSSL_API_COMPAT=%)
	BROKER_CPPFLAGS:=$(BROKER_CPPFLAGS) -DWITH_TLS $(APP_OPENSSL_API_COMPAT:%=-DOPENSSL_API_COMPAT=%)
	BROKER_LDADD:=$(BROKER_LDADD) -lssl -lcrypto
	CLIENT_CPPFLAGS:=$(CLIENT_CPPFLAGS) -DWITH_TLS $(APP_OPENSSL_API_COMPAT:%=-DOPENSSL_API_COMPAT=%)
	LIB_CPPFLAGS:=$(LIB_CPPFLAGS) -DWITH_TLS $(APP_OPENSSL_API_COMPAT:%=-DOPENSSL_API_COMPAT=%)
	LIB_LIBADD:=$(LIB_LIBADD) -lssl -lcrypto
	PASSWD_LDADD:=$(PASSWD_LDADD) -lssl -lcrypto
